import { isComplianceInsightEventType } from "../adapters/aimi.adapter";
import type { ComplianceAimiInsightItem } from "../compliance-dashboard.interface";
import { asField, payloadRecord } from "../compliance-dashboard.repository";

export function buildComplianceAimiInsights(
  tenant_id: string,
  events: readonly Readonly<Record<string, unknown>>[],
): readonly ComplianceAimiInsightItem[] {
  const items: ComplianceAimiInsightItem[] = [];
  let index = 0;
  while (index < events.length) {
    const row = events[index];
    if (asField(row, "tenant_id") === tenant_id) {
      const event_type = asField(row, "event_type");
      if (isComplianceInsightEventType(event_type) === true) {
