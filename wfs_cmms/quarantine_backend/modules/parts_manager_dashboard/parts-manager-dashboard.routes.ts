...
import { PARTS_MANAGER_API_ROUTES } from "./api/parts-manager-dashboard.api.contract";
import { createPartsManagerDashboardController } from "./parts-manager-dashboard.controller";
...
export function createPartsManagerDashboardRouter(service: PartsManagerDashboardService) {
  const controller = createPartsManagerDashboardController(service);
  return async function partsManagerDashboardRouter(req: IncomingMessage, res: ServerResponse, dto: ContextDto): Promise<boolean> {
    ...
    let index = 0;
    while (index < PARTS_MANAGER_API_ROUTES.length) {
      const route = PARTS_MANAGER_API_ROUTES[index];
