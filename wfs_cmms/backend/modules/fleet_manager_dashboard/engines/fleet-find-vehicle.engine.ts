import { gpsSource, telematicsBreadcrumbSource } from "../adapters/telematics.adapter";
...
export function buildFindVehicle(
  ...
  while (index < telematics.length) {
    ...
    if (asField(row, "tenant_id") === tenant_id) {
      ...
      if (include === true) {
        const source = asField(row, "source") === "gps" ? gpsSource() : telematicsBreadcrumbSource();
        points.push(
          Object.freeze({
            tenant_id,
            asset_id,
            ...
            fault_code: asField(row, "fault_code"),
            severity,
            source,
