...
import { canAccessSilentMasterKeyDashboard } from "./silent-master-key-dashboard.rbac";
import { loadSilentMasterKeySession } from "./hooks/useSilentMasterKeyApi";

export default function SilentMasterKeyDashboardLayout(props: { readonly children: ReactNode }) {
  const [allowed, setAllowed] = useState(false);
  const [ready, setReady] = useState(false);
  const [tenant_id, setTenant] = useState("");

  useEffect(() => {
    const session = loadSilentMasterKeySession();
    if (session === null) {
      setAllowed(false);
      setReady(true);
      return;
    ...
    setAllowed(canAccessSilentMasterKeyDashboard(session.role) && session.tenant_id !== "");
    setReady(true);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#05010d] text-[#f5f3ff]" style={shellStyle}>
      <header style={barStyle}>
        <p style={brandStyle}>Silent Master Key Dashboard</p>
