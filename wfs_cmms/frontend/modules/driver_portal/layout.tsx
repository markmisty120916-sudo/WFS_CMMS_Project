...
import { canAccessDriverPortal } from "./driver-portal.rbac";
import { loadDriverPortalSession } from "./hooks/useDriverPortalApi";

export default function DriverPortalLayout(props: { readonly children: ReactNode }) {
  const [allowed, setAllowed] = useState(false);
  const [ready, setReady] = useState(false);
  const [tenant_id, setTenant] = useState("");

  useEffect(() => {
    const session = loadDriverPortalSession();
    if (session === null) {
      setAllowed(false);
      setReady(true);
      return;
    ...
    setAllowed(canAccessDriverPortal(session.role) && session.tenant_id !== "");
    setReady(true);
  }, []);
  ...
  if (allowed === false) {
    return (
      <div className="min-h-screen bg-[#05010d]" style={shellStyle}>
        <p style={deniedStyle}>{driverPortalLabel("en", "denied")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#05010d] text-[#f5f3ff]" style={shellStyle}>
      <header style={barStyle}>
        <p style={brandStyle}>{driverPortalLabel("en", "portal")}</p>
...
const mainStyle: CSSProperties = { padding: "16px", maxWidth: "640px", margin: "0 auto" };
