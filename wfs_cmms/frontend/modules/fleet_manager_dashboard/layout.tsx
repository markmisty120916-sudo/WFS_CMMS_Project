...
import { loadFleetManagerSession } from "./hooks/useFleetManagerApi";

export default function FleetManagerDashboardLayout(props: { readonly children: ReactNode }) {
  const [allowed, setAllowed] = useState(false);
  const [ready, setReady] = useState(false);
  const [tenant_id, setTenant] = useState("");

  useEffect(() => {
    const session = loadFleetManagerSession();
    if (session === null) {
      setAllowed(false);
      setReady(true);
      return;
    ...
    setAllowed(canAccessFleetManagerDashboard(session.role) && session.tenant_id !== "");
    setReady(true);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#05010d] text-[#f5f3ff]" style={shellStyle}>
      <header style={barStyle}>
        <p style={brandStyle}>Fleet Manager Dashboard</p>
