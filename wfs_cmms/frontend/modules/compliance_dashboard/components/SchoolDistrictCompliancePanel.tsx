...
export function SchoolDistrictCompliancePanel(props: { readonly filter: ComplianceDashboardFilter }) {
  const api = useComplianceDashboardApi(props.filter);
  const [items, setItems] = useState<readonly ComplianceDistrictItem[]>([]);

  useEffect(() => {
    void (async () => {
      const payload = (await api.request(api.routes.district)) as readonly ComplianceDistrictItem[] | null;
  ...
  return (
    <section className="rounded-xl border border-violet-600 p-4" style={panelStyle}>
      <h2 style={titleStyle}>{complianceDashboardWidgetLabel("district")}</h2>
      {items.map((item) => (
        <article key={item.inspection_id} style={cardStyle}>
          <p style={accentStyle}>{item.type}</p>
          <p style={mutedStyle}>
            {item.inspection_id} {item.status} asset {item.asset_id}
          </p>
        </article>
      ))}
    </section>
