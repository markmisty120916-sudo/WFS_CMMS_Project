...
export function ComplianceFindingsPanel(props: { readonly filter: ComplianceDashboardFilter }) {
  const api = useComplianceDashboardApi(props.filter);
  const [items, setItems] = useState<readonly ComplianceFindingItem[]>([]);

  useEffect(() => {
    void (async () => {
      const payload = (await api.request(api.routes.findings)) as readonly ComplianceFindingItem[] | null;
  ...
  return (
    <section className="rounded-xl border border-violet-600 p-4" style={panelStyle}>
      <h2 style={titleStyle}>{complianceDashboardWidgetLabel("findings")}</h2>
      {items.map((item) => (
        <article key={item.inspection_id} style={cardStyle}>
          <p style={accentStyle}>{item.status}</p>
          <p style={mutedStyle}>
            asset {item.asset_id} repeat {item.repeat_offender}
          </p>
        </article>
      ))}
    </section>
