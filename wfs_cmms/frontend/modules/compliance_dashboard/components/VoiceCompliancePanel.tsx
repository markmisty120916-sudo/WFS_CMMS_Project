...
import type { ComplianceDashboardFilter, ComplianceVoiceItem } from "../compliance-dashboard.interface";
...
export function VoiceCompliancePanel(props: { readonly filter: ComplianceDashboardFilter }) {
  const api = useComplianceDashboardApi(props.filter);
  const [items, setItems] = useState<readonly ComplianceVoiceItem[]>([]);

  useEffect(() => {
    void (async () => {
      const payload = (await api.request(api.routes.voice)) as readonly ComplianceVoiceItem[] | null;
  ...
  return (
    <section className="rounded-xl border border-violet-600 p-4" style={panelStyle}>
      <h2 style={titleStyle}>{complianceDashboardWidgetLabel("voice")}</h2>
      {items.map((item) => (
