...
import { loadPartsManagerSession } from "./hooks/usePartsManagerApi";

export default function PartsManagerDashboardLayout(props: { readonly children: ReactNode }) {
  const [allowed, setAllowed] = useState(false);
  const [ready, setReady] = useState(false);
  const [tenant_id, setTenant] = useState("");

  useEffect(() => {
    const session = loadPartsManagerSession();
    if (session === null) {
      setAllowed(false);
      setReady(true);
      return;
    ...
    setAllowed(canAccessPartsManagerDashboard(session.role) && session.tenant_id !== "");
    setReady(true);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#05010d] text-[#f5f3ff]" style={shellStyle}>
      <header style={barStyle}>
        <p style={brandStyle}>Parts Manager Dashboard</p>
