...
import type { TechnicianLanguage, TechnicianSession } from "../technician.interface";
...
export function isTechnicianLanguage(value: string): value is TechnicianLanguage {
  if (value === "English") {
    return true;
  }
  if (value === "Spanish") {
...
export function loadTechnicianSession(): TechnicianSession | null {
...
export function normalizeUnknownArray(payload: unknown): readonly unknown[] {
  ...
  const nestedKeys = ["items", "data", "workorders", "assets", "schedules", "requests", "events", "insights"];
  let index = 0;
