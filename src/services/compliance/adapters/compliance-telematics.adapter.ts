...
import { asFieldString, normalizeTenantId } from "../utils/compliance-normalizer";

export type ComplianceTelematicsSignal = {
...
export class ComplianceTelematicsAdapter {
  ...
  async loadSignals(asset_id: string): Promise<readonly ComplianceTelematicsSignal[] | null> {
    if (asset_id === "") {
      return null;
    }
    ...
    const rows: ComplianceTelematicsSignal[] = [];
    let index = 0;
  ...
  faultLevel(signals: readonly ComplianceTelematicsSignal[]): TelematicsFaultLevel {
    let index = 0;
  ...
  clusterLevel(signals: readonly ComplianceTelematicsSignal[]): TelematicsClusterLevel {
