...
import { asFieldString, normalizeTenantId } from "../utils/pm-normalizer";

export type PmTelematicsSignal = {
...
export class PmTelematicsAdapter {
  ...
  async loadSignals(asset_id: string): Promise<readonly PmTelematicsSignal[] | null> {
    if (asset_id === "") {
      return null;
    }
  ...
  faultLevel(signals: readonly PmTelematicsSignal[]): TelematicsFaultLevel {
    let index = 0;
  ...
  clusterLevel(signals: readonly PmTelematicsSignal[]): TelematicsClusterLevel {
