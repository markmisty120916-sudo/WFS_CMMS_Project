...
import { asFieldString, normalizeTenantId } from "../utils/driver-defect-normalizer";

export type DriverDefectTelematicsSignal = {
...
export class DriverDefectTelematicsAdapter {
  ...
  async loadSignals(asset_id: string): Promise<readonly DriverDefectTelematicsSignal[] | null> {
    if (asset_id === "") {
      return null;
    }
    ...
    const rows: DriverDefectTelematicsSignal[] = [];
    let index = 0;
  ...
  faultLevel(signals: readonly DriverDefectTelematicsSignal[]): TelematicsFaultLevel {
    let index = 0;
  ...
  clusterLevel(signals: readonly DriverDefectTelematicsSignal[]): TelematicsClusterLevel {
